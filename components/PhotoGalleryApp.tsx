...
const AlbumCard: React.FC<{ album: PhotoAlbum; onClick: () => void }> = memo(({ album, onClick }) => (
    <button onClick={onClick} className="bg-slate-800 rounded-lg shadow-md w-full text-left group overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-blue-500">
...
const PhotoCard: React.FC<{ photo: Photo; onClick: () => void; onDelete: (photoId: string) => void; }> = memo(({ photo, onClick, onDelete }) => (
    <div className="relative group aspect-square">
        <button onClick={onClick} className="w-full h-full bg-slate-800 rounded-md overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-blue-500">
        ...
        <div className="p-4 md:p-8">
            <header className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Álbuns de recordações dos seus eventos.</p>
                </div>
                 <button
